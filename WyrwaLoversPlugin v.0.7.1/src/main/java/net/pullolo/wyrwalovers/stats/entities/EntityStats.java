...
public class EntityStats extends EntityStatistics{
    ...
    public EntityStats(Entity e, String type){
        ...
        if (!(e instanceof Player)){
            if (e.getType().equals(EntityType.ZOMBIE)){
                isUndead = true;
                setLevel(rand.nextInt(levelScale+10)+1);
                updateDefaults();
            }
                ...
                setLevel(rand.nextInt(levelScale+10)+6);
                updateDefaults();
            }
            else if (e.getType().equals(EntityType.ENDERMAN)){
                setLevel(rand.nextInt(levelScale+10)+11);
                updateDefaults();
            }
            else if (e.getType().equals(EntityType.PHANTOM)){
                setLevel(rand.nextInt(levelScale)+11);
                ...
                updateCustom(200*getLevel(), 100, 200, 0, 100, getLevel()*DAMAGE_MULTIPLIER*7, 50, 100, 0, 0);
            }
            else {
                setLevel(rand.nextInt(levelScale+10)+1);
                updateDefaults();
            }
            if (entityType.equalsIgnoreCase("lapis_zombie")){
                setLevel(10);
                updateCustom(200.0, 2, 50, 50, 100, 100, 10, 100, 0, 0);
    ...
    private void runChecks(Entity e){
        ...
        if (e.getLocation().getY() < -100){
            if (e instanceof Damageable){
                ((Damageable) e).damage(0.1);
            }
            setHealth(0.0);
        }
    }
