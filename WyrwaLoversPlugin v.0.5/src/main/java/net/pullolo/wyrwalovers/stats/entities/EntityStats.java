    ...
    public EntityStats(Entity e, String type){
        ...
        if (!(e instanceof Player)){
            if (e.getType().equals(EntityType.ZOMBIE)){
                ...
                setLevel(rand.nextInt(10)+6);
                updateDefaults();
            }
            else if (e.getType().equals(EntityType.ENDERMAN)){
                setLevel(rand.nextInt(20)+11);
                updateDefaults();
            }
            else {
                setLevel(rand.nextInt(3)+1);
                updateDefaults();
            }
            calcStats();
            heal();
    ...
    public void dealDamage(Entity e, Double amou, boolean isPhysical, boolean isCrit, double critDamageAmount) {
        ...
        if (isPhysical){
            if (getDefense() < 10){
                ...
                setHealth(getHealth() - amount/getDefense()*10);
                new DamageIndicator(e, amount/getDefense()*10, isCrit);
            }
        } else {
            if (getResistance() < 10){
                setHealth(getHealth() - amount);
                ...
                setHealth(getHealth() - amount/getResistance()*10);
                new DamageIndicator(e, amount/getResistance()*10, isCrit);
            }
        }
    }
